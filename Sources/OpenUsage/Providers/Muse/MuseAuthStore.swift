...
enum MuseAuthError: Error, LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .notLoggedIn:
            return "Sign in to dev.meta.ai in your browser (or save the llm_sess cookie with the API key editor) and try again."
        case .browserBlocked:
            return "OpenUsage couldn't read your browser's cookies. Grant it Full Disk Access in System Settings → Privacy & Security and try again."
        case .saveFailed:
            return "Couldn't save the Muse session cookie."
        case .deleteFailed:
            return "Couldn't remove the saved Muse session cookie."
        }
    }
}

/// How a browser-cookie lookup ended: found, nothing stored, or present-but-unreadable (macOS
/// denied the read — Full Disk Access missing). Callers report `unreadable` distinctly so a
/// permission problem never renders as "not logged in".
...
/// Browsers probed for the `llm_sess` session cookie, in fallback order after the default browser.
...
/// Resolves the `llm_sess` cookie that authenticates the dev.meta.ai usage page.
/// Checked in order, first hit wins:
///
/// 1. A manually saved cookie: `~/.config/muse-usage/session`, then
///    the `MUSE_LLM_SESS` environment variable. This is also what the Settings API-key editor
///    manages, so a user without Full Disk Access can paste the cookie once from DevTools.
/// 2. The default browser's cookie store, then every other installed browser. Chromium cookies
///    are Keychain-encrypted (`<Browser> Safe Storage`); Firefox stores them in cleartext;
///    Safari uses `Cookies.binarycookies`.
///
/// The manual path never touches the keychain or a browser, so it works with zero permissions.
struct MuseAuthStore: Sendable {
    ...
    func loadManualCookie() -> MuseAuth? {
        keyStore.loadKey().map(MuseAuth.init(sessionCookie:))
    }

    func currentAPIKey() -> String? { keyStore.loadKey() }
